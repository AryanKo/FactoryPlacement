...
run_test(
    ...
    and "location" in openapi.text
    and "indicators" in openapi.text,
)
...
run_test(
    "Test 6 - Happy Path",
    res.status_code == 200
    and "location" in res.json()
    and "surface_water_trend" in res.json()["indicators"],
)
...
run_test(
    "Test 7 - Contract", all(c in ["measured", "no_data"] for c in confidence_values)
)
...
run_test(
    "Test 11 - Mock Mode",
    res.json()["indicators"]["surface_water_trend"]["value"] == -12.4,
)
...
run_test(
    "Test 12 - Production Mode",
    res.json()["indicators"]["surface_water_trend"]["value"] is None
    and res.json()["indicators"]["surface_water_trend"]["confidence"] == "no_data",
)
