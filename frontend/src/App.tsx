...
import { fetchRiskData, fetchExplanation, USE_MOCK_API } from './api/client';
...
import { ShieldCheck, Droplet, Code, Activity, AlertCircle, Navigation, MapPin } from 'lucide-react';
...
export const App: React.FC = () => {
  ...
  // Sequenced execution pipeline: location select / map click -> GET /api/risk -> POST /api/explain
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-[#090d16] text-gray-100 selection:bg-blue-500 selection:text-white">
      ...
      <header className="fixed top-0 left-0 right-0 h-16 z-[3500] bg-slate-950/90 backdrop-blur-2xl border-b border-white/10 px-4 md:px-6 flex items-center justify-between gap-4 shadow-2xl">
        {/* Left: Brand Logo & Title */}
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className="p-2 rounded-xl bg-blue-500/20 border border-blue-400/40 text-blue-400 shadow-lg shadow-blue-950">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] uppercase font-mono px-2 py-0.5 rounded bg-blue-500/20 text-blue-300 border border-blue-500/40 font-bold hidden sm:inline-block">
                Gemma 4 Shield
              </span>
            </div>
        ...
        {/* Center: Search Autocomplete & Location Preset Pills */}
        <div className="hidden md:flex items-center gap-3 flex-1 max-w-xl">
          {apiKey ? (
            ...
            <div className="flex-1 text-xs text-gray-400 font-mono bg-slate-900 px-3 py-1.5 rounded-xl border border-white/10">
              Set VITE_GOOGLE_MAPS_API_KEY for Places Autocomplete
            </div>
          )}
            ...
            {PRESET_LOCATIONS.map((preset) => {
              ...
              return (
                <button
                  ...
                  className={`text-[10px] px-2.5 py-1 rounded-full border whitespace-nowrap flex items-center gap-1 transition-all ${
                    isSelected
        ...
        {/* Right: View Mode Tabs & Dev Status */}
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <div className="hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded-xl glass-panel border border-white/10 text-[11px] font-mono text-emerald-400">
            <span className="w-2 h-2 rounded-full bg-emerald-400 animate-ping"></span>
            {USE_MOCK_API ? 'Dev Mock Mode' : 'Live Backend'}
          </div>
        </div>
      </header>
      ...
      {error && (
        <div className="fixed top-20 left-1/2 -translate-x-1/2 z-[4500] max-w-lg w-full glass-panel p-3 bg-red-950/90 border border-red-500/40 text-red-200 text-xs flex items-center gap-3 shadow-2xl">
      ...
      <aside className="fixed top-16 right-0 bottom-0 w-full sm:w-96 md:w-[420px] z-[3000] bg-slate-950/90 backdrop-blur-2xl border-l border-white/10 p-4 overflow-y-auto space-y-4 shadow-2xl scrollbar-thin">
      ...
      {activeTab === 'compare' && (
        <div className="fixed inset-0 z-[4000] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-6 pt-20">
      ...
      {activeTab === 'contracts' && (
        <div className="fixed inset-0 z-[4000] bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-6 pt-20 overflow-y-auto">
